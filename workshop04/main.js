...
const etagFunction = {
	stateAsync: (req) => {
		const state = req.para.state.toLowerCase();
		const p = new Promise(
			(resolve,reject) => {
...
db.getDB()
	.then((db) => {
		...
		app.listen(PORT, () => {
			...
			console.info(`Registring serivce ${serviceName}:${serviceId}`)
			consul.agent.service.register({
				...
				check: {
					ttl: '10s',
					deregisterscriticalserviceafter: '30s'
				}
			})
			.then(() => {
				...
				process.on('SIGINT', 
					() => {
						console.info(`Deregistring service: ${serviceName}:${serviceId}`);
						clearInterval(intId);
